"use client";

import Link from "next/link";
import { notFound } from "next/navigation";

import { DressBoardView } from "@/components/rsvp/dress-board";
import { EditableImage, EditableText } from "@/components/rsvp/editable";
import { useEventStore } from "@/components/rsvp/event-store";
import { TocPager } from "@/components/rsvp/toc";
import { DayChip, PageShell, SectionPanel } from "@/components/rsvp/ui";

export function DayView({ dayId }: Readonly<{ dayId: string }>) {
  const { content } = useEventStore();
  const day = content.schedule.days.find((d) => d.id === dayId);
  if (!day) {
    notFound();
  }
  const dayIdx = content.schedule.days.findIndex((d) => d.id === dayId);
  const dayPathPrefix = `schedule.days[${dayIdx}]`;

  const dressBoard = day.dressBoardId
    ? content.dressBoards.find((b) => b.id === day.dressBoardId)
    : undefined;

  const days = content.schedule.days;
  const idx = days.findIndex((d) => d.id === day.id);
  const prev = idx > 0 ? days[idx - 1] : null;
  const next = idx >= 0 && idx < days.length - 1 ? days[idx + 1] : null;

  const dayNav = (
    <nav
      aria-label="Day navigation"
      className="flex flex-wrap items-center justify-between gap-3"
    >
      {prev ? (
        <Link
          href={`/rsvp/day/${prev.id}`}
          className="rsvp-btn rsvp-btn-ghost text-xs sm:text-sm"
        >
          ← {prev.dayLabel} · {prev.dayName}
        </Link>
      ) : (
        <Link
          href="/rsvp/overview"
          className="rsvp-btn rsvp-btn-ghost text-xs sm:text-sm"
        >
          ← Back to overview
        </Link>
      )}
      {next ? (
        <Link
          href={`/rsvp/day/${next.id}`}
          className="rsvp-btn rsvp-btn-neon text-xs sm:text-sm"
        >
          {next.dayLabel} · {next.dayName} →
        </Link>
      ) : (
        <Link
          href="/rsvp/rsvp"
          className="rsvp-btn rsvp-btn-primary text-xs sm:text-sm"
        >
          Fill out the RSVP →
        </Link>
      )}
    </nav>
  );

  const scheduleRows = (
    <div className="grid gap-3">
      {day.rows.map((row, rowIdx) => (
        <div
          key={row.id}
          className="grid grid-cols-[auto_1fr] items-start gap-4 rounded-2xl border border-[var(--rsvp-border-soft)] bg-[rgba(10,4,18,0.5)] px-4 py-4 sm:grid-cols-[8rem_1fr]"
        >
          <EditableText
            path={`${dayPathPrefix}.rows[${rowIdx}].time`}
            value={row.time}
            className="font-[var(--font-bebas-neue)] text-lg tracking-[0.18em] text-[var(--rsvp-teal)]"
          />
          <div>
            <EditableText
              path={`${dayPathPrefix}.rows[${rowIdx}].title`}
              value={row.title}
              as="p"
              className="text-base font-semibold text-[var(--rsvp-ink)]"
            />
            <EditableText
              path={`${dayPathPrefix}.rows[${rowIdx}].note`}
              value={row.note ?? ""}
              as="p"
              placeholder="+ note"
              className="mt-1 text-xs uppercase tracking-[0.22em] text-[var(--rsvp-gold)]"
            />
          </div>
        </div>
      ))}
    </div>
  );

  const tipBlock = (
    <div className="rounded-[1.4rem] border border-[var(--rsvp-pink)]/30 bg-[rgba(255,61,154,0.08)] px-6 py-6">
      <EditableText
        path={`${dayPathPrefix}.tipTitle`}
        value={day.tipTitle}
        as="p"
        className="font-[var(--font-bebas-neue)] text-lg tracking-[0.22em] text-[var(--rsvp-pink-soft)]"
      />
      <EditableText
        path={`${dayPathPrefix}.tipBody`}
        value={day.tipBody}
        as="p"
        multiline
        className="mt-3 text-sm leading-7 text-[var(--rsvp-ink)]"
      />
    </div>
  );

  const hasHero = Boolean(day.heroImageUrl);

  return (
    <PageShell>
      {dayNav}

      {/* Hero header: photo above the schedule, with title overlaid.
          Falls back to the original centered title when no image is set. */}
      {hasHero ? (
        <section className="rsvp-panel rounded-[2rem] p-0 overflow-hidden">
          <div className="relative isolate">
            <EditableImage
              urlPath={`${dayPathPrefix}.heroImageUrl`}
              altPath={`${dayPathPrefix}.heroImageAlt`}
              url={day.heroImageUrl}
              alt={day.heroImageAlt ?? day.dayName}
              className="h-72 w-full object-cover sm:h-[22rem] lg:h-[26rem]"
            />
            <div
              aria-hidden="true"
              className="pointer-events-none absolute inset-0"
              style={{
                background:
                  "linear-gradient(180deg, rgba(7,4,10,0.35) 0%, rgba(7,4,10,0.15) 45%, rgba(7,4,10,0.95) 100%)",
              }}
            />
            <div className="pointer-events-none absolute inset-x-0 bottom-0 flex flex-col items-center gap-3 px-6 pb-8 text-center sm:gap-4 sm:pb-10">
              <span className="rsvp-eyebrow pointer-events-auto">Daily Schedule</span>
              <DayChip tone="gold">
                <span className="pointer-events-auto">
                  <EditableText
                    path={`${dayPathPrefix}.dayLabel`}
                    value={day.dayLabel}
                  />
                </span>
              </DayChip>
              <h1
                className="pointer-events-auto font-[var(--font-bebas-neue)] text-[2.6rem] leading-[0.9] tracking-[0.04em] text-[var(--rsvp-gold)] sm:text-[4rem]"
                style={{ textShadow: "0 2px 18px rgba(0,0,0,0.85)" }}
              >
                <EditableText
                  path={`${dayPathPrefix}.dayName`}
                  value={day.dayName}
                />
              </h1>
            </div>
          </div>
          <div className="px-6 pt-8 pb-2 text-center sm:px-10">
            <span className="rsvp-divider mx-auto" aria-hidden="true" />
            <EditableText
              path={`${dayPathPrefix}.headline`}
              value={day.headline}
              as="h2"
              className="mt-6 font-[var(--font-playfair-display)] text-2xl text-[var(--rsvp-pink-soft)] sm:text-3xl"
            />
            <EditableText
              path={`${dayPathPrefix}.intro`}
              value={day.intro}
              as="p"
              multiline
              className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[var(--rsvp-ink)] sm:text-base"
            />
          </div>

          <div className="mt-8 px-6 pb-6 sm:px-10 sm:pb-8">{scheduleRows}</div>

          <div className="mx-6 mb-8 sm:mx-10">{tipBlock}</div>
        </section>
      ) : (
        <SectionPanel>
          <div className="flex flex-col items-center text-center">
            <span className="rsvp-eyebrow">Daily Schedule</span>
            <div className="mt-5 flex flex-col items-center gap-4">
              <DayChip tone="gold">
                <EditableText
                  path={`${dayPathPrefix}.dayLabel`}
                  value={day.dayLabel}
                />
              </DayChip>
              <h1 className="font-[var(--font-bebas-neue)] text-[3rem] leading-[0.9] tracking-[0.04em] text-[var(--rsvp-gold)] sm:text-[4rem]">
                <EditableText
                  path={`${dayPathPrefix}.dayName`}
                  value={day.dayName}
                />
              </h1>
            </div>
            <span className="mt-5 rsvp-divider" aria-hidden="true" />
            <EditableText
              path={`${dayPathPrefix}.headline`}
              value={day.headline}
              as="h2"
              className="mt-6 font-[var(--font-playfair-display)] text-2xl text-[var(--rsvp-pink-soft)] sm:text-3xl"
            />
            <EditableText
              path={`${dayPathPrefix}.intro`}
              value={day.intro}
              as="p"
              multiline
              className="mt-4 max-w-2xl text-sm leading-7 text-[var(--rsvp-ink)] sm:text-base"
            />
          </div>

          <div className="mt-10">{scheduleRows}</div>

          <div className="mt-10">{tipBlock}</div>
        </SectionPanel>
      )}

      {dressBoard ? <DressBoardView board={dressBoard} /> : null}

      <TocPager />
    </PageShell>
  );
}
