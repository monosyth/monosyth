  ...
  return (
    <section className="glass-panel rounded-[1.75rem] px-5 py-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em]">
            Shape your private Monosyth workspace.
          </h2>
        </div>
        <p className="text-sm leading-7 text-stone-600">
          These fields save into your private Firestore `users/{'{uid}'}` document.
        </p>
      </div>
      ...
      <form className="mt-6 grid gap-4" onSubmit={handleSubmit}>
        ...
        <label className="grid gap-2 text-sm text-stone-700">
          <span className="font-medium">Studio notes</span>
          <textarea
            value={form.bio}
            onChange={(event) =>
              setForm((current) => ({
            ...
            placeholder="Private notes about what this space is for and what you want to build next."
          />
        </label>

        <label className="grid gap-2 text-sm text-stone-700">
          <span className="font-medium">Working links</span>
          <textarea
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {profileSaveState === "saving" ? "Saving..." : "Save settings"}
          </button>
          {profileSaveMessage ? (
