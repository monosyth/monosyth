...
export function MoveHeading({ setup, today, saved }: { setup: MoveSetup; today: string; saved: boolean }) {
  ...
  const destination = setup.destination.split(",")[0].trim();
  const origin = setup.origin.split(",")[0].trim();
  return (
      ...
      <div className={styles.moveRoute}>
        ...
        <h1 id="plan-heading" className={styles.routeHeading}>
          {destination ? <>
            ...
            <span className={styles.routeDestination}><span aria-hidden="true">↗</span>{destination}.</span>
          </> : "Your next chapter."}
        </h1>
